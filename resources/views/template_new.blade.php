        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
            ...
            <a href="#" class="dropdown-item">
              <a href="{{route('dashbord/login')}}" class="dropdown-item dropdown-footer">Keluar</a>
            </div>
          </li>
        </ul>
      </nav>
      ...
      <aside class="main-sidebar sidebar-light-primary elevation-4">
        ...
        <div class="sidebar">
          ...
          <nav class="mt-2">
            ...
            <li class="nav-item">
              <a href="<?=URL::to($res->dashbord)?>" class="nav-link">
            ...
            <?php 
              $user=DB::table('tbl_user_faskes')
              ->select('nik_admin','admin')
              ->where('id_user',Session::get('nik_admin'))
              ->first();
              //,DB::raw('COUNT(master_menu.issue_id) as followers')
              $menu_nav=DB::table('user_menu')->select('*')->where('id_user','=',Session::get('nik_admin'))->get();
              $label_menu = DB::table('master_menu')
                          // ->join('user_menu', 'user_menu.id_menu', '=', 'master_menu.id')
                          ->select('master_menu.*')
                          ->where('parent','=','true')
                          ->get();
              $hitung = DB::table('master_menu')
                          ->join('user_menu', 'user_menu.id_menu', '=', 'master_menu.id')
                          ->select('master_menu.parent')
                          ->distinct()
                          // ->where('parent','=','true')
                          ->where('id_user','=',Session::get('nik_admin'))
                          ->get();$a=0;
              foreach($hitung as $htg){
              $label_menu_ = DB::table('master_menu')
                          // ->join('user_menu', 'user_menu.id_menu', '=', 'master_menu.id')
                          ->select('master_menu.*')
                          // ->where('parent','=','true')
                          ->where('id','=',$htg->parent)
              ...
              foreach($label_menu_ as $lm){$a++;
                  $count = DB::table('user_menu')->count();
              ?>
